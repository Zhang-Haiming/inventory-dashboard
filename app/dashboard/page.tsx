    ...
    <div className="min-h-screen flex flex-col">
      ...
      {!isLoading && (
        <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-4">
          ...
          <div className="pb-8">
            {activeTab === 'summary' && (
              <MonthlySummary stockIn={stockIn} stockOut={stockOut} />
            )}
            {activeTab === 'inventory' && (
              <CategoryInventory stockIn={stockIn} stockOut={stockOut} thresholds={thresholds} />
            )}
            {activeTab === 'alerts' && (
              <LowStockAlert stockIn={stockIn} stockOut={stockOut} thresholds={thresholds} />
            )}
            {activeTab === 'stockin' && (
              <StockInTable
                rows={stockIn}
                onUpdate={updateStockInRow}
                onDelete={deleteStockInRow}
                onAdd={addStockInRow}
              />
            )}
            {activeTab === 'stockout' && (
              <StockOutTable
                rows={stockOut}
                onUpdate={updateStockOutRow}
                onDelete={deleteStockOutRow}
                onAdd={addStockOutRow}
              />
            )}
          </div>
        </div>
