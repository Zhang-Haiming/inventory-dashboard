...
export function UploadModal({ open, onClose, onImport, parseAndImport }: Props) {
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg bg-green-50 border border-green-200">
              ...
              <div className="text-sm">
                ...
                <p className="text-green-600 mt-0.5">
                  入库记录 {preview.stats.stockInCount} 条，出库记录 {preview.stats.stockOutCount} 条
                </p>
              </div>
            </div>
            {preview.warnings.length > 0 && (
              <div className="rounded-lg bg-amber-50 border border-amber-200 p-3">
                <p className="text-xs font-medium text-amber-700 mb-1">注意：</p>
                {preview.warnings.map((w, i) => <p key={i} className="text-xs text-amber-600">· {w}</p>)}
              </div>
            )}
            <p className="text-xs text-slate-400">确认导入后，已有数据将被替换。之前设置的低库存阈值会保留。</p>
          </div>
        )}

        <DialogFooter className="gap-2">
          <Button variant="outline" onClick={handleClose}>取消</Button>
          {state === 'preview' && <Button onClick={handleConfirm}>确认导入</Button>}
          {state === 'error' && <Button variant="outline" onClick={reset}>重新选择</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
